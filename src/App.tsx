    ...
    <div className="flex h-screen bg-[#0f172a] text-slate-200 overflow-hidden font-sans">
      ...
      >
        <div className="p-6 flex items-center gap-3">
          <div className="w-8 h-8 bg-purple-600 rounded-lg flex items-center justify-center font-bold text-xl">S</div>
          {isSidebarOpen && <span className="font-bold text-xl tracking-tight text-white">SmartAssist</span>}
        </div>
        ...
        <nav className="flex-1 px-3 space-y-1 mt-4">
          {navItems.map((item) => (
            ...
            >
              <item.icon className={cn("w-5 h-5", activeView === item.id ? "text-white" : "group-hover:text-cyan-400")} />
      ...
      <main className="flex-1 overflow-y-auto relative bg-[#0f172a]">
        ...
        <header className="sticky top-0 z-40 bg-[#0f172a]/80 backdrop-blur-md px-8 py-6 flex justify-between items-center border-b border-slate-800/50">
          <div>
            <h2 className="text-sm font-medium text-slate-400 uppercase tracking-widest">{formatDate(currentTime.toISOString())}</h2>
            <div className="flex items-center gap-2 mt-1">
              <h1 className="text-3xl font-bold text-white tracking-tight">{getGreeting()}, User</h1>
              <span className="text-3xl font-light text-slate-500">|</span>
              <span className="text-2xl font-mono text-cyan-400">{currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeView === 'proceedings' && (
                <ProceedingsView
                  meetings={meetings}
                  onUpdateMeeting={(updated) => setMeetings(prev => prev.map(m => m.id === updated.id ? updated : m))}
                />
              )}
              {activeView === 'voice' && (
                <VoiceAssistantPanel 
